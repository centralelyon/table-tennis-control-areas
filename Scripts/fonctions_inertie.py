...
from Scripts.fonctions_time import time_to_point
from Scripts.fonctions_positions import coordonnees,orientation
...
def heatmap(match, point, tlim, que_taille_zone='non', num_vitesse=1): # taille de la zone de chaleur du joueurA + création des zones de chaleur frame par frame de chaque joueur pour un point donné dans un match donné (image stockée dans répertoire 'heatmap' dans le dossier du match dans tt-espace)
...
# tlim correspond au temps seuil : si le joueur atteint un point dans l'espace en + de tlim alors on considère que ce point ne va pas être atteint donc il ne sera pas coloré en rouge.
    os.path.join(chemin_tt_espace,"example",match,point)
    if not os.path.isdir(os.path.join(chemin_tt_espace,"example",match,point,"enveloppe")):
        os.mkdir(os.path.join(chemin_tt_espace,"example",match,point,"enveloppe"))
    if not os.path.isdir(os.path.join(chemin_tt_espace,"example",match,point,"heatmap")):
        os.mkdir(os.path.join(chemin_tt_espace,"example",match,point,"heatmap"))
    if not os.path.isdir(os.path.join(chemin_tt_espace,"example",match,point,"heatmap_sans")):
        os.mkdir(os.path.join(chemin_tt_espace,"example",match,point,"heatmap_sans"))
    chemin_csv = os.path.join(chemin_tt_espace,"example",match,point,point+"_annotation.csv")
    df = pd.read_csv(chemin_csv)
    
    pos_init_b, vitesse_b, pos_init_jA, vitesse_jA, pos_init_jB, vitesse_jB = coordonnees(match, point, lissage='oui', num_vitesse=num_vitesse)
    orientationJA,orientationJB = orientation(match, point, pos_init_jA, lissage='non', joint=[11,14], multiplicateur=50)
    ...
    field=Image.open(os.path.join(chemin_tt_espace,"images","utils","environnement_table.png")).convert('RGB')
    W, h = field.size
    c=10
    ...
    if que_taille_zone=='non':
        for f in range(len(pos_init_b)):
            ...
            field_frame.save(os.path.join(chemin_tt_espace,"example",match,point,"heatmap","heatmap_frame_"+str(f)+".png"))
            field_frame.save(os.path.join(chemin_tt_espace,"example",match,point,"heatmap_sans","image_"+str(f)+".png"))
            ...
            heatmap_with_contours.save(os.path.join(chemin_tt_espace,"example",match,point,"heatmap","zone_heatmap_frame_"+str(f)+".png"))
            ...
            result_image.save(os.path.join(chemin_tt_espace,"example",match,point,"heatmap","image_"+str(f)+".png"))
            

            width, height = 800, 800

            joueurA = match.split("_vs_")[0]#"ALEXIS-LEBRUN"
            effet = "topspin"

            IMAGE_PATH = os.path.join(chemin_tt_espace,"example",match,point,"heatmap","heatmap_frame_"+str(f)+".png")
            ...
            df_revers = pd.read_csv(os.path.join(chemin_tt_espace,"example",match,"enveloppe",match+"_"+joueurA+"_revers_"+effet+"_enveloppe.csv"))[['X', 'Y']]#.values.astype(np.int32)
            df_coup_droit = pd.read_csv(os.path.join(chemin_tt_espace,"example",match,"enveloppe",match+"_"+joueurA+"_coup_droit_"+effet+"_enveloppe.csv"))[['X', 'Y']]#.values.astype(np.int32)
            ...
            chemin_enregistrement = os.path.join(chemin_tt_espace,"example",match,point,"enveloppe",str(f)+".png")
            ...
            if f in df['time_frappe'].values:
                ...
                chemin_enregistrement2 = os.path.join(chemin_tt_espace,"example",match,point,"enveloppe",str(f)+"_frappe.png")
