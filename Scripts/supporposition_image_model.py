...
def calculer_camera_params(points_2d, image_size):
    ...
    if not success:
        raise ValueError("Error solvePnP : Can't compute camera position")
...
def creation_gif_zone_image_proj(dossier_point,dossier_image_modele,table,table_proj_sol,pts_image2):
    ...
    for i in range(sum(1 for nom_fichier in os.listdir(os.path.join(dossier_point,"heatmap")) if nom_fichier.startswith("image_"))):
