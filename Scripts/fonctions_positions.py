...
def coordonnees(match, point, lissage='non',num_vitesse=1): # dans le repère tkinter (x vertical, y horizontal descendant, en haut à gauche de l'image)
    """
    Entrée:
            - compet
            - match
    ...
    """
    chemin_video = os.path.join(chemin_tt_espace,"example",match,point,point+".mp4")
    cap=cv2.VideoCapture(chemin_video)
    FPS=cap.get(cv2.CAP_PROP_FPS)
    
    with open(os.path.join(chemin_tt_espace,"example",match,point,"csv_json_openpose",point+"_zone_joueur_avec_pos_balle_3D.csv"), newline='') as fichier_openpose :
        openpose=[]
...
def orientation(match, point, pos_init_jA, lissage='non', joint=[11,14], multiplicateur=1): # dans le repère tkinter (x vertical, y horizontal descendant, en haut à gauche de l'image)
    """
    Function used to compute players' orientations by using 2 keypoints of the pose estimation
    Args:
            - match: name of match
            - point: name of point (set_?_point_?)
            - pos_init_jA (used to know players' sides)
            - lissage
            - joints (tuple of keypoints)
    Returns:
            - pos_init_jA (player A position)
            - vitesse_jA (player A orientation)
            - pos_init_jB (player B position)
            - vitesse_jB (player B orientation)
    """

    cap=cv2.VideoCapture(os.path.join(chemin_tt_espace,"example",match,match+".mp4"))
    FPS=cap.get(cv2.CAP_PROP_FPS)
    csv_pose = pd.read_csv(os.path.join(chemin_tt_espace,"example",match,point,"csv_json_openpose",point+"_openpose.csv"))
    
    
    point_table = recuperation_points_table(os.path.join(chemin_tt_espace,"example",match,match+"_perspective.json"))
    ...
    for i in range(0,nb_frame+1):
        ...
        chemin_dossier_enregistrement = os.path.join(chemin_tt_espace,"example",match,"test_orientation")
...
def ajout_vecteurs_trajectoires(match, point, images, faire_orientation=False): # sur les frames d'un point, ajout de la trajectoire des 2 joueurs (ie positions prises aux frames précédentes) + leur vecteur vitesse à l'instant t
    """Fonction used to add trajectory vectors to images

    Args:
        match (_type_): _description_
        point (_type_): _description_
        images (_type_): _description_
        faire_orientation (bool, optional): _description_. Defaults to False.
    """
    ...
    os.makedirs(os.path.join(chemin_tt_espace,"example",match,point), exist_ok='True')

    coord_balle, vitesse_balle, coord_jA, vitesse_jA, coord_jB, vitesse_jB = coordonnees(match, point) # dans ce code pas besoin des vitesses
    orientationJA,orientationJB = orientation(match, point, coord_jA, lissage='non', joint=[11, 14], multiplicateur=50)
    ...
    for i in range(1,len(coord_balle)):
        ...
        image=cv2.imread(os.path.join(chemin_tt_espace,"example",match,point,"output","image_"+str(i)+".png"))
...
def atteintes(match, precision_espace='non', precision_frappe='non'): # positions relatives sur un match complet de la balle au moment de la frappe par rapport au joueur qui la frappe (en mettant 'oui' pour les précisions, on peut récupérer le pourcentage de frappes effectuées dans la zone rouge de la heatmap (inertie) ou pourcentage de frappes dans la banane)
    ...
    for i in range(len(set_point)):
        point = "set_"+str(set_point[i][0])+"_point_"+str(set_point[i][1])
        chemin=os.path.join(chemin_tt_espace,"example",match,point)

        with open(os.path.join(chemin,point+"_annotation.csv"), newline='') as fichier :
        ...
        with open(os.path.join(chemin_tt_espace,"example",match,point,"csv_json_openpose",point+"_zone_joueur_avec_pos_balle.csv"), newline='') as fichier :
            set_point_pose=[]
            reader=csv.reader(fichier)
            for ligne in reader :
        ...
        if precision_espace=='oui' or precision_frappe=='oui' :
            ...
            if cote_j[set_point[i][0]-1][1]=='bas':
                vitesse_jA=coordonnees(match, point)[3]
            elif cote_j[set_point[i][0]-1][1]=='haut':
                vitesse_jA=[]
                for j in range(nb_frappes_par_pt):
                    vitesse_jA.append([-coordonnees(match, point)[3][j][0], -coordonnees(match, point)[3][j][1]])
...
def atteintes_lateralite(match): # idem que les 2 fonctions du dessus sauf qu'en plus on sait si le joueur a frappé la balle en coup droit ou en revers (3ème élément de chaque sous-liste)
    with open(os.path.join(chemin_tt_espace,"example",match,match+"_annotation_enrichi.csv"), newline='') as fichier_annotations :
        annotations=[]
        reader=csv.reader(fichier_annotations)
        for ligne in reader :
    ...
    coord_jB=[]
    for i in range(len(set_point)):
        point = "set_"+str(set_point[i][0])+"_poin_"+str(set_point[i][1])
        chemin=os.path.join(chemin_tt_espace,"example",match,point)
        ...
        with open(os.path.join(chemin,"csv_json_openpose",point+"zone_joueur_avec_pos_balle.csv"), newline='') as fichier :
            set_point_pose=[]
            reader=csv.reader(fichier)
            for ligne in reader :
                set_point_pose.append(ligne)
...
def banane_alexis(match, surface='non'): # on récupère le contour de la zone de frappe (nommée 'banane' en raison de sa forme) d'Alexis Lebrun sur le match contre Ma Long à Singapour (cette banane a pour vocation d'être utilisée sur d'autres matchs avec Alexis, on a choisi arbitrairement ce match pour la tracer)
    atteintes_jA, atteintes_jB = atteintes(match)
...
def banane_alexis_cd_r(match): # en complément de la fonction du dessus : la banane est la zone globale dans laquelle se trouve toutes les frappes du match. or on peut raffiner cette zone puisqu'il y a 4 sous-zones qui se distinguent : 2 zones revers et 2 zones coup droit. cette fonction donne les coordonnées du contour de ces 4 zones.

    atteintes_jA = atteintes_lateralite(match)[0]
...
def angles_avant_bras(match, point): # angles de chaque joueur au cours d'un point entre ses avant-bras et l'axe des y (dans la longueur de la table). trouvé par calcul avec l'openpose par Aymeric (il doit y avoir une erreur dans son code puisque les angles ne sont pas du tout conformes à ce qu'on peut observer)
    with open(os.path.join(chemin_tt_espace,"example",match,point,"csv_json_openpose",point+"_angles.csv"), newline='') as fichier :
        set_point_angle=[]
        reader=csv.reader(fichier)
        for ligne in reader :
