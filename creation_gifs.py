...
from fonctions_inertie import heatmap
from fonctions_positions import coordonnees, angles_avant_bras, ajout_vecteurs_trajectoires, banane_alexis, banane_alexis_cd_r
from fonctions_time import frappe
from generation_grille_table import quadrillage_sur_image
...
def gif_point(compet, match, set, point, avec_heatmap='non', avec_banane='non', num_vitesse=1): # /!\ avec_banane='oui' ssi le joueurA est Alexis Lebrun (car la banane utilisée est la sienne). si d'autres bananes sont générées à posteriori, le code peut être simplement décliné pour des joueurs donnés.
    ...
    pos_init_b = coordonnees(compet, match, set, point, lissage='oui', num_vitesse=num_vitesse)[0]

    if avec_heatmap=='non':
        ajout_vecteurs_trajectoires(compet, match, set, point, [None, '{}/environnement_table.png'.format(chemin_tt_espace)])
    elif avec_heatmap=='oui':
        heatmap(compet, match, set, point, 0.8, que_taille_zone='non', num_vitesse=num_vitesse)
        os.makedirs('{}/output/match_{}/heatmap/quadrillage'.format(chemin_tt_espace, match), exist_ok='True')
        for f in range(len(pos_init_b)):
            quadrillage_sur_image('{}/output/match_{}/heatmap/heatmap_frame_{}.png'.format(chemin_tt_espace, match, f), '{}/output/match_{}/heatmap/quadrillage'.format(chemin_tt_espace, match), 'heatmap_quadrillage_{}'.format(f))
        ...
        ajout_vecteurs_trajectoires(compet, match, set, point, [os.path.join(USER_PREFERENCE["pipeline"]["LOCAL-PIPELINE-TT"],compet,match,"clips","set_"+str(set)+"_point_"+str(point),"heatmap"), 'image_{}.png'],False)

    if avec_banane=='oui': # dessin zone de frappe
        pos_init_jA = coordonnees(compet, match, set, point, lissage='oui')[2]

        banane=banane_alexis() # /!\ pour la banane, les coordonnées sont donnnées comme : [[x, y], [x, y], ...] // clusters :  [[liste_des_x], [liste_des_y]]
    ...
    for i in range(len(time_frappe)):
        image = Image.open('{}/output/match_{}/point_{}/image_{}.png'.format(chemin_tt_espace, match, point, time_frappe[i][1])).convert('RGB')
        draw=ImageDraw.Draw(image)
        x = int(time_frappe[i][2][0]) + 400
        y = -int(time_frappe[i][2][1]) + 400
        draw.ellipse((x-10, y-10, x+10, y+10), fill='brown')
        image.save('{}/output/match_{}/point_{}/image_{}.png'.format(chemin_tt_espace, match, point, time_frappe[i][1]))
    # ajout de la position de la frappe de SERVICE d'après 'annotation.csv' (n'est pas comprise dans 'time_frappe')
    image = Image.open('{}/output/match_{}/point_{}/image_{}.png'.format(chemin_tt_espace, match, point, frappe_service[1])).convert('RGB')
    draw=ImageDraw.Draw(image)
    x = int(frappe_service[2][0]) + 400
    y = -int(frappe_service[2][1]) + 400
    draw.ellipse((x-10, y-10, x+10, y+10), fill='brown')
    image.save('{}/output/match_{}/point_{}/image_{}.png'.format(chemin_tt_espace, match, point, frappe_service[1]))
    ...
    frames = np.stack([iio.imread('{}/output/match_{}/point_{}/image_{}.png'.format(chemin_tt_espace, match, point, i)) for i in range(len(pos_init_b))], axis = 0)
    iio.mimwrite(os.path.join('{}/output/match_{}/point_{}'.format(chemin_tt_espace, match, point), 'gif.gif'), frames, fps=FPS)
